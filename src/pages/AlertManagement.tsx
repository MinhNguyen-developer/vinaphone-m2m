...
const AlertManagement: React.FC = () => {
  ...
  console.log("Triggered alerts:", triggeredList);
    ...
    },
    {
      ...
      render: (_: unknown, record: AlertConfig) => {
        ...
        if (record.productCode)
          return <Tag color="blue">Mã: {record.productCode}</Tag>;
  ...
  const items = [
      ...
      children: (
        <>
          ...
          {triggeredLoading ? (
            ...
            <Alert
              message="✅ Không có SIM nào vượt ngưỡng cảnh báo."
              type="success"
              showIcon
            />
          ) : (
            <Card>
              <Alert
                message={`⚠️ ${triggeredList.length} SIM đang vượt ngưỡng. Cần kiểm tra!`}
                type="error"
                showIcon
    ...
    },
    {
      ...
      label: (
        <span>
          <BellOutlined /> Cấu hình cảnh báo ({alerts.length})
        </span>
      ),
      children: (
